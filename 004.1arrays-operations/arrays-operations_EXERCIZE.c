...
void bubble_sort(int* array, int dimension);
...
#define ONE_GIGABYTE (1024 * 1024 * 1024L)
	...
	// è come se avessi scritto che ptr conitene l'indirizzo &alloc_test[0]
	printf("alloc_test[4] = %lu\n", alloc_test[3]);
	while (*ptr != -1) {
...
int main(int argc, char *argv[]) {
	...
	int * int_array;
	// allocare (creare) un array
	int_array = malloc(size * sizeof(int)); // dimensione totale in bytes dell'array? 10 *4 = 40
	...
	// OPERIAMO SU COPIA_ARRAY, quando abbiamo finito lo liberiamo
	free(copia_array);
	...
	// puntatore a char definito precedentemente
	...
	//int* int_array; se lo metto faccio una ri-dichiarazione
	int_array = calloc(ARRAY_SIZE,sizeof(int)); // INIZIALIZZA ANCHE A 0
	...
	printf("contenuto dell'array int_array:\n");
	for (i = 0; i < ARRAY_SIZE; i++) {
		printf("%d ", int_array[i]);
	}
	printf("\n");

	complete_reverse_int(int_array,ARRAY_SIZE);
	...
	// FATTO MA CONTROLLARE CHE FUNZIONI
	...
	char char_array_2[] = {'C','i','a','o'};
	char* con_array;

	con_array = concat_arrays(char_array_1, sizeof(char_array_1),
			char_array_2,sizeof(char_array_2));
	printf("risultato della concatenazione: \n %s\n", con_array);
	// stringhe di caratteri: array di caratteri terminati dal char di valore 0 : '\0'

	// ESERCIZIO: esempio con bubble sort

	int example[ARRAY_SIZE] = {10,5,-6,3,2,7,-5,9,0,76};
	...
	bubble_sort(example,ARRAY_SIZE);
...
//	while (i < j)
	...
	// IL "WHLIE" SOPRA EQUAIVALE A
	while (i < j){
		j--;
		swap(&array[i], &array[j]);
		i++;

		//scambia le celle di cui gli passo gli indirizzi fino a che quella i-esima non rimane
		// inferiore a quella j-esima
...
char * concat_arrays(char * array1, int array1_dimension, char * array2, int array2_dimension) {
	...
	char* new_array;
	new_array = malloc((array1_dimension+array2_dimension)* sizeof(char));
	...
	for (int i = 0; i<array2_dimension;i++){
		new_array[array1_dimension+i] =array2[i];
	}
...
void bubble_sort(int * array, int array_dimension) {
	...
	printf("Number of operation (algo 1): %d\n", num_op1);

	/*
	// METODO 2 equivalente
	int num_op2 = 0;
	for (int i = 0; i<array_dimension; i++){
		for (int j = i+1; j<array_dimension; j++){
			if (array[i]>array[j]){
				swap_int(&array[i], &array[j]);
				num_op2++;
			}
	...
	printf("Number of operation (algo 2): %d \n", num_op2);
	*/
}
